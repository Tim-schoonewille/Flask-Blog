...
def register():
    ...
    if register_form.validate_on_submit():
        ...
        hashed_password = generate_password_hash(register_form.password.data,
                                                 ...
                                                 salt_length=8)
        fetch_mail = User.query.filter_by(email=register_form.email.data).first()

        if fetch_mail:
            flash('Mail already in database!')
...
def login():
    ...
    if form.validate_on_submit():
        ...
        check_password = check_password_hash(pwhash=hashed_db_password,password=form.password.data)
        if check_password:
            login_user(user)
            return redirect(url_for('get_all_posts'))
        else:
